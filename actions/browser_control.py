...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".rex_profiles" / browser
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".rex_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using REX profile")
                rex_profile = str(Path.home() / ".rex_profiles" / "firefox_rex")
                Path(rex_profile).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(rex_profile, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".rex_profiles" / "safari")
        ...
        rex_profile = str(Path.home() / ".rex_profiles" / self.browser_name)
        Path(rex_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with REX profile: {rex_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(rex_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with REX profile")
