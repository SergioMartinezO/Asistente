...
class SysMetricsTracker:
    ...
    # ── Umbrales para alertas proactivas ─────────────────────────────────────
    CPU_WARN  = 85.0   # % CPU — alerta
    CPU_CRIT  = 95.0   # % CPU — crítico
    MEM_WARN  = 80.0   # % RAM — alerta
    MEM_CRIT  = 92.0   # % RAM — crítico
    TMP_WARN  = 80.0   # °C   — alerta
    TMP_CRIT  = 92.0   # °C   — crítico

    def check_alerts(self) -> list[dict]:
        """Devuelve lista de alertas activas con nivel (warn/crit) y métrica."""
        snap = self.snapshot()
        alerts = []

        cpu = snap["cpu"]
        if cpu >= self.CPU_CRIT:
            alerts.append({"metric": "CPU", "value": cpu, "level": "crit",
                           "msg": f"CPU al {cpu:.0f}% — riesgo de colapso. Cerrando procesos no críticos."})
        elif cpu >= self.CPU_WARN:
            alerts.append({"metric": "CPU", "value": cpu, "level": "warn",
                           "msg": f"CPU al {cpu:.0f}% — carga elevada."})

        mem = snap["mem"]
        if mem >= self.MEM_CRIT:
            alerts.append({"metric": "RAM", "value": mem, "level": "crit",
                           "msg": f"RAM al {mem:.0f}% — memoria crítica."})
        elif mem >= self.MEM_WARN:
            alerts.append({"metric": "RAM", "value": mem, "level": "warn",
                           "msg": f"RAM al {mem:.0f}% — memoria elevada."})

        tmp = snap["tmp"]
        if tmp > 0:
            if tmp >= self.TMP_CRIT:
                alerts.append({"metric": "TEMP", "value": tmp, "level": "crit",
                               "msg": f"Temperatura CPU {tmp:.0f}°C — riesgo térmico crítico."})
            elif tmp >= self.TMP_WARN:
                alerts.append({"metric": "TEMP", "value": tmp, "level": "warn",
                               "msg": f"Temperatura CPU {tmp:.0f}°C — temperatura elevada."})
        return alerts
