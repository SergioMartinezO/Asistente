...
TOOL_DECLARATIONS = [
    ...
    {
        "name": "remember_technical",
        "description": (
            "Guarda una solución técnica, hallazgo o conocimiento de ingeniería en la memoria técnica "
            "de largo plazo, organizado por dominio (electronica, mecatronica, software, soluciones). "
            "Úsalo cuando el usuario resuelve un problema técnico, descubre la causa raíz de un error, "
            "o documenta un procedimiento que podría reutilizarse. "
            "No anunciar que se guarda — hacerlo silenciosamente."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "key":      {"type": "STRING", "description": "Título corto del conocimiento (ej: 'fix_dpi_qt6_windows')"},
                "value":    {"type": "STRING", "description": "Descripción de la solución o hallazgo (máx 500 chars)"},
                "category": {
                    "type": "STRING",
                    "description": "electronica | mecatronica | software | soluciones (se auto-detecta si se omite)"
                },
            },
            "required": ["key", "value"]
        }
    ...
    {
        "name": "recall_technical",
        "description": (
            "Recupera soluciones técnicas previas relevantes de la memoria de largo plazo "
            "buscando por similitud de palabras clave. "
            "Úsalo antes de responder preguntas técnicas complejas para ver si hay "
            "soluciones previas documentadas del mismo tipo de problema."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "query": {"type": "STRING", "description": "Descripción del problema o tema a buscar"},
            },
            "required": ["query"]
        }
    },
